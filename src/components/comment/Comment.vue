        ...
        <div class="comment-info">
            ...
            <div class="info-item comment-others flex-based-container">
                ...
                <div class="thumbs-up common-based-btn flex-based-container" @click="changeThumbsUpStatus()">
                    <img v-if="!getCommentInfo.isLiked" alt="未点赞" src="@/assets/img/comment/no_thumbsUp.svg"
                        style="width: 1rem;" />
                    <img v-else src="@/assets/img/comment/yes_thumbsUp.svg" alt="已点赞" style="width: 1rem;" />
                    <p>{{ getCommentInfo.likeCount }}</p>
                </div>
...
const changeThumbsUpStatus = async () => {
    // getCommentInfo.value.isLiked = !getCommentInfo.value.isLiked // 更改目前的前端状态
    // 调用后端接口
    if (props.commentInfo.isLiked) {
      if (await deleteLike(userInfo.getId(), getCommentInfo.value.id, 'comment')) {
        ElMessage.success('取消点赞成功')
        getCommentInfo.value.isLiked = false
        getCommentInfo.value.likeCount--
      }else {
        ElMessage.error('取消点赞失败')
        getCommentInfo.value.isLiked = true
      }

    } else {
      if (await addLike(userInfo.getId(), getCommentInfo.value.id, 'comment')) { // 测试
